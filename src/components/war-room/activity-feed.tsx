...
export function ActivityFeed({ updates }: ActivityFeedProps) {
  const getIcon = (type: string) => {
    switch (type) {
      case 'insert': return <ArrowUpRight className="h-4 w-4 text-status-confirmed" />;
      case 'update': return <RefreshCcw className="h-4 w-4 text-status-neutral" />;
      case 'delete': return <Trash2 className="h-4 w-4 text-status-critical" />;
      default: return <Zap className="h-4 w-4 text-status-warning" />;
    }
  };
  ...
  const getChannelColor = (channel: string) => {
    switch (channel) {
      ...
      default: return 'bg-status-neutral/10 text-status-neutral border-status-neutral/20';
    }
  };

  return (
    <Card className="bg-surface-1 border-border flex flex-col h-full shadow-2xl">
          ...
          <div className="space-y-0">
            {updates.length === 0 ? (
              ...
              updates.map((update, idx) => (
                <div 
                  key={update.id} 
                  className={`flex gap-4 p-4 border-b border-border hover:bg-surface-2/50 transition-colors ${idx === 0 ? 'bg-primary/5' : ''}`}
                >
                  <div className="mt-1">
                    {getIcon(update.type)}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-foreground/80 leading-tight">
                      <span className="font-bold text-foreground capitalize">{update.type}</span> detected on system entry.
                    </p>
                    <div className="mt-2 flex items-center gap-4">
                      ...
                      <div className="numeric text-[10px] text-primary/60">
